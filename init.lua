...
core.register_node("mysoundblocks:block", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if core.get_player_privs(placer:get_player_name()).mysoundblocks ~= true then
			core.chat_send_player(placer:get_player_name(), "You need the mysoundblocks priv")
			return
		end
	...
	on_dig = function(pos, node, player)
		if core.get_player_privs(player:get_player_name()).mysoundblocks ~= true then
			core.chat_send_player(player:get_player_name(), "You need the mysoundblocks priv")
			return
		end
	...
	on_rightclick = function(pos, node, player, itemstack, pointed_thing)
		...
		core.show_formspec(
			...
			"size[6,7;]"
				...
				.. dd
				.. "]"
				.. "field[1,3;2,1;sndl;Length;"
				.. bb
				.. "]"
				.. "field[3.5,3;2,1;sndhd;Radius;"
				.. cc
				.. "]"
				.. "label[0.7,3.4;Player or All]"
				.. "dropdown[0.7,3.8;2,1;pora;Player,All;]"
				.. "field[3.5,4;2,1;snddis;Hear Distance;"
				.. ee
				.. "]"
				.. "field[2.75,5;1,1;sndgn;Gain;"
				.. ff
				.. "]"
		...
		core.register_on_player_receive_fields(function(player2, formname, fields)
			...
			if fields["ents"] or fields["entc"] or fields["entb"] or fields["snd"] or fields["txt"] then
				if fields["ents"] and fields["snd"] ~= "" then
					meta2:set_string("a", thing1)
					meta2:set_string("b", thing2)
					meta2:set_string("c", thing3)
					meta2:set_string("d", thing4)
					meta2:set_string("e", "sound")
					meta2:set_string("f", thing6)
					meta2:set_string("g", thing7)
					meta2:set_string("h", thing8)

					core.swap_node(pos, { name = "mysoundblocks:block_hidden" })

					return true
				elseif fields["entc"] and fields["txt"] ~= "" then
					meta2:set_string("a", thing1)
					meta2:set_string("b", thing2)
					meta2:set_string("c", thing3)
					meta2:set_string("d", thing4)
					meta2:set_string("e", "chat")

					core.swap_node(pos, { name = "mysoundblocks:block_hidden" })

					return true
				elseif fields["entb"] and fields["txt"] ~= "" and fields["snd"] ~= "" then
					meta2:set_string("a", thing1)
					meta2:set_string("b", thing2)
					meta2:set_string("c", thing3)
					meta2:set_string("d", thing4)
					meta2:set_string("e", "both")
					meta2:set_string("f", thing6)
					meta2:set_string("g", thing7)
					meta2:set_string("h", thing8)
		...
		for _, obj in ipairs(all_objects) do
			if obj:is_player() then
				...
				if not player_name[p] then
					...
					if player_name[p] ~= blockname then
						player_name[p] = nil
					end

					-- play sound for everyone to hear
					if sound_pa == "All" and glob == nil then
					...
					-- display text
					if (sound_chat == "chat" or sound_chat == "both") and block_text and block_text ~= chats[p] then
						core.chat_send_player(p, block_text)
						chats[p] = block_text
					end

					-- play sound
					if (sound_chat == "sound" or sound_chat == "both") and block_sound then
						-- stop any sounds still playing
						if handler[p] and block_sound ~= sound[p] then
							core.sound_stop(handler[p])
							if player_name[p] then
								core.chat_send_all(player_name[p] .. " and " .. block_sound)
							end
						end
					...
					core.after(block_time, function(p2)
						player_name[p2] = nil
					end, p)
				end
			end
